...
//Change return value if needed
{
	...
	outEdgeRH.insert(RHMap::value_type(rnode, hTuple));
	...
	for (it = r->terms.begin(); it != r->terms.end(); it++)
	{
		...
		if (functor != NULL)
		{
			...
			outEdgesTR[bTuple].push_back(rnode);
			continue;
		}
...
DPGraph::PrintGraph()
{
	...
	RHMap::iterator itr;
	for (itr = outEdgeRH.begin();itr != outEdgeRH.end();itr++)
	{
		//Print the rule node
		itr->first->PrintNode();
		cout << endl;
		...
		cout << "Head tuple:";
		itr->second->PrintName();
		cout << endl;
	}

	cout << "Tuple outgoing edges:" << endl;
	TRMap::iterator itt;
	for (itt = outEdgesTR.begin();itt != outEdgesTR.end();itt++)
	{
		//Print the tuple node
		itt->first->PrintNode();
		cout << endl;

		//Print the rule nodes connected by the tuple node
		RuleVec::iterator itrv;
